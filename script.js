...
// Importação do react-beautiful-dnd via esm.sh, tentando forçar a dependência do React
...
// MODIFICADO: Elementos raiz para React
...
let reactRoot = null; 
let notificationReactRoot = null; // Raiz separada para notificações

// Dados Iniciais Simulados (mantidos)
...
// --- Componentes React ---
// (Card, Modal, AddItemForm, List, BoardDisplay, AdminPanel, MapPanel, AssociateDrawingForm, MarkingsListPanel - como nas versões anteriores)
// ... (O código dos componentes React permanece o mesmo da versão anterior) ...
// COPIE E COLE OS COMPONENTES REACT DA VERSÃO ANTERIOR AQUI
// ... (Funções Card, Modal, AddItemForm, List, BoardDisplay, AdminPanel, MapPanel, AssociateDrawingForm, MarkingsListPanel)
                ...
                React.createElement('main', { className: 'main-content-area' },
                    React.createElement('div', { className: `content-view ${currentView === 'board' ? 'visible' : 'hidden'}`},
                        currentView === 'board' && React.createElement(BoardDisplay, { 
                            ...
                            currentUserProfile: board.users.find(u => u.username === loggedInUser)?.profile || "Global", // Passa o perfil do utilizador logado
                            isAdmin: isAdmin
                        })
                    ),
        ...
        loginFormEl.addEventListener('submit', function(event) {
            ...
            if ((usernameInput === "admin" && password === "admin123") || (usernameInput === "usuario@teste.com" && password === "senha123")) {
                ...
                dashboardAreaEl.style.display = 'block'; 
                document.body.style.backgroundColor = '#0079BF'; 
                document.body.style.overflow = 'hidden'; 

                if (!reactRoot) {
                    reactRoot = ReactDOM.createRoot(dashboardAreaEl);
                }
