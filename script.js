...
import { getFirestore, doc, getDoc, addDoc, setDoc, updateDoc, deleteDoc, onSnapshot, collection, query, where, orderBy, serverTimestamp, getDocs, writeBatch } from 'firebase/firestore';
...
// ============================================================================
// Componentes do Trello Clone
// ============================================================================

function TrelloBoards({ userId, showUserMessage, confirmAction, appId, db }) {
...
function Board({ boardId, userId, showUserMessage, confirmAction, appId, db, onUpdateBoardBackground, currentBoard }) {
  ...
  const handleBackgroundChange = (color) => {
    onUpdateBoardBackground(boardId, color);
...
// Novo componente: Modal de Detalhes do Cartão
function CardDetailModal({ card, boardId, listId, userId, showUserMessage, onClose, appId, db, confirmAction }) { // Receive confirmAction prop
  ...
  const [editedCoverImage, setEditedCoverImage] = useState(card.coverImage || ''); // Novo estado para capa
  const [checklistItems, setChecklistItems] = useState(card.checklist || []); // Novo estado para checklist
      ...
      <div className="bg-white p-8 rounded-xl shadow-2xl max-w-3xl w-full border border-gray-200 relative overflow-y-auto max-h-[90vh]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="space-y-4">
            ...
            {card.lastEditedAt && (
                <p className="text-xs text-gray-500">Última edição: {new Date(card.lastEditedAt.toDate()).toLocaleString()} por {card.lastEditedBy.substring(0, 6)}...</p>
            )}
          </div>
